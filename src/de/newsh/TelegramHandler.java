...
public class TelegramHandler {
	...
	public static void updateTimerBot(String timerName, long duration, boolean addJob) {


		try {
			
			JsonParser parser = new JsonParser();
			String telegramChatID = new String(Files.readAllBytes(Paths.get("telegramBot.json"))); // read from json file in root
			...
			if (Pattern.matches("[0-9,-]+", telegramChatID)) { // chatID must only contain numbers. '-' at start indicates groupChat
				...
				String jsonString = "{\"TimerName\":\"" + timerName + "\",\"Duration\":\"" + duration + "\",\"ChatID\":\"" + telegramChatID + "\"}"; // build json String to send to script
				try {
